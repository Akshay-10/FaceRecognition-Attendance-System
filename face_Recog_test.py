...
class realpredict:
    def __init__(self, root):
        self.root = root
        self.an = self.root.winfo_screenwidth()
        self.al = self.root.winfo_screenheight()
...
#OOOOOOOOO
        ...
        attnsbutton=customtkinter.CTkButton(RFrame1,command=self.attnsmark,width=625,height=30,text="ATTENDANCE",text_font=("Tahoma", 15, "bold"),fg_color="#fb341c")
        ...
        leftframe.place(x=10,y=20,width=700,height=300)

        img1 = Image.open("img\\featured_image-1.jpg")
        img1 = img1.resize((700, 300), Image.Resampling.LANCZOS)
        self.photoimg4 = ImageTk.PhotoImage(img1)
        bg_img1 = customtkinter.CTkLabel(leftframe, image=self.photoimg4,fg_color="black",bg_color="black")
        ...
        self.detailtbl = ttk.Treeview(down_frame, columns=("EMPLOYEE_ID", "NAME", "DATE", "TIME"), xscrollcommand=topBtmsbar.set,yscrollcommand=sideBar.set)
                    ...
                    for i in range(len(comparison)):
                        if comparison[i]==True:
                            ...
                            for i in prev_attendance:
                                ...
                                cv2.putText(check_img,emp_name,(10,400),font,2,(255,255,255),4,cv2.LINE_AA)
                                check=cv2.cvtColor(check_img,cv2.COLOR_BGR2RGB)
                                cv2.imshow(emp_name,check);
                                mixer.music.play()
                                #time.sleep(10)
